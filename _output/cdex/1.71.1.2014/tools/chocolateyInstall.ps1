...
$installerType = 'EXE'
$url = 'http://softlayer-ams.dl.sourceforge.net/project/cdexos/cdexos/CDex 1.72/CDex-1.72-win32.exe'
$silentArgs = '/S'
...
try {
	...
	if ($installedVersion -gt $version) {
		...
		Install-ChocolateyPackage "$packageName" "$installerType" "$silentArgs" "$url"  -validExitCodes $validExitCodes
		  ...
		  $is64bit = $processor.AddressWidth -eq 64
		  if ($is64bit) {
			$exeLocation = "${Env:ProgramFiles(x86)}\CDex"
		  } else {
			$exeLocation = "$Env:ProgramFiles\CDex"
		  }
