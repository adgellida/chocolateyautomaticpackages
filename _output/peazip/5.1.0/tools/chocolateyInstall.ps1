...
$processor = Get-WmiObject Win32_Processor
  ...
  if ($is64bit) {
    $url = 'http://surfnet.dl.sourceforge.net/project/peazip/5.1.0/peazip-5.1.0.WIN64.exe'
    #$url = 'http://surfnet.dl.sourceforge.net/project/peazip/{version}/peazip-{version}.WIN64.exe'
  } else {
    $url = 'http://surfnet.dl.sourceforge.net/project/peazip/5.1.0/peazip-5.1.0.WINDOWS.exe'
